...
class scintilla {
    ...
    __call(func, param*){
        ...
        {
            wParam:=param[1],lParam:=param[2], hwnd:=param[3] ; get current parameters
            ; check if we are passing text in unicode build, if so convert to ANSI
            ; and store pointer because sendEditor only works with pointers
            (a_isunicode && wParam && !(wParam+1)) ? (VarSetCapacity(wParamA, StrPut(wParam, "CP0"))
                                                     ,StrPut(wParam, &wParamA, "CP0"), wParam:=&wParamA) 
                                                   : NULL
            (a_isunicode && lParam && !(lParam+1)) ? (VarSetCapacity(lParamA, StrPut(lParam, "CP0"))
                                                     ,StrPut(lParam, &lParamA, "CP0"), lParam:=&lParamA) 
                                                   : NULL

            ; check if we are going to pass a pointer to a buffer to be filled by scintilla and 
            ; set the capacity of the variable accordingly
            _functs:="gettext,getline,getseltext,getcurline,autocgetcurrenttext"
            if func in %_functs%
                VarSetCapacity(str:="", wParam * (a_isunicode ? 2 : 1)),buffer := lParam, lParam:=&str, buf:=true
            
            res := SCI_sendEditor(hwnd, func, wParam, lParam)

            buf ? (strput(nstr:=strget(lParam), buffer, strput(nstr, "CP0")), buf:=false) : NULL
            ; if func in %_functs%
                ; msgbox % strget(buffer, "CP0")
            return res
        }
    }
}
...
/*
    ...
    (end)
*/
Add(hParent, x=5, y=15, w=390, h=270, Styles="", MsgHandler="", DllPath=""){
